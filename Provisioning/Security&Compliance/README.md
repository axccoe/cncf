...
<img src="./image/keycloak.png" alt="" width="100"/>  
   ...
   - **특징**  
     ...
     - 노코드 기반 : 완벽하게 사용자 지정 가능한 로그인 페이지, 강력한 인증, 비밀번호 복구 지원, 클라이언트 비밀 로테이션, 2FA 복구 코드, 약관, 단계별 인증 등을 제공. 코딩 필요 없이 가능
...
### 8. **Notary** (CNCF Incubating)  
<img src="./image/notary.png" alt="" width="100"/>  

   - **설명**: 아티팩트 서명 및 검증. 개발부터 배포까지 소프트웨어 제공 보안을 보호합니다. Notary Project는 정품 컨테이너 이미지와 기타 OCI 아티팩트를 사용하여 소프트웨어 공급망을 보호하기 위한 산업 간 표준을 제공하기 위한 사양 및 도구 세트입니다. Notation Project 사양 및 도구는 OCI 아티팩트에 대한 서명 및 검증 워크플로, OCI 호환 레지스트리 간 서명 이식성, 플러그인 모델을 통한 타사 키 관리 솔루션과의 통합을 제공합니다.
   - **문제 해결**: Notary는 소프트웨어 공급망의 보안을 강화해 컨테이너 이미지와 아티팩트가 배포 과정에서 변조되거나 손상되지 않도록 방지합니다. 이 외에도 이미지 및 아티팩트 무결성 검증, 서명 및 검증 프로세스 간소화에 도움을 줍니다. 
   - **특징**  
     - 무결성 보장 : Notary는 서명된 컨테이너 이미지 및 아티팩트를 제공해, 배포자가 전달한 데이터가 원본 그대로임을 보장
     - 신뢰할 수 있는 배포 : 개발자는 서명된 아티팩트만을 배포하여, 소프트웨어의 안전한 전달을 보장 가능
     - TUF(Trusted Updater Framework) 기반 : Notary는 TUF의 보안 모델을 채택해, 이미지 및 아티팩트의 무결성을 보호하는 강력한 보안 체계를 제공
     - 다중 서명 지원 : 하나의 이미지 또는 아티팩트에 대해 여러 서명을 지원해, 팀이나 여러 조직 간의 협업을 위한 신뢰성을 보장
     - 자동화된 서명 검증 : 사용자는 컨테이너 이미지나 소프트웨어를 배포하기 전에 자동으로 서명을 확인하여 신뢰할 수 있는 소프트웨어만을 실행 가능
   - **사용 사례**: Docker 이미지에 서명해, Kubernetes 클러스터와 같은 환경에서 서명된 이미지만을 신뢰하고 배포할 수 있습니다. Notary는 소프트웨어 패키지 업데이트 시 서명을 검증해, 신뢰할 수 없는 소스에서 온 업데이트를 방지합니다. 패키지 관리자와 통합하여, 개발자가 패키지에 서명을 부여하고, 배포할 때 패키지가 변조되지 않았는지 확인할 수 있습니다. 패키지 관리자는 이를 통해 신뢰할 수 있는 패키지만을 배포할 수 있습니다.
   - [Notary 문서](https://notaryproject.dev/)